 ...
 * Caixas quotidiennes, envoi de cadeaux, inventaire luxe
 * Thème Maserati 🏎️👑✨🇨🇮
...
// ── CAIXAS PRESTIGE DISPONIBLES ──
const CAIXAS_MASERATI = {
  commune: {
    id: 'commune',
    nom: '📦 Coffret Bleu Nuit',
    cout: 0,
    delai: 24 * 60 * 60 * 1000, // 24h – une seule par jour
    recompenses: [
...
const CADEAUX_ENVOYABLES = {
  ...
  ourson:    { id: 'ourson', emoji: '🧸', nom: 'Ours en Bleu Nuit', cout: 200, message: 'un ours en peluche luxe' },
  diamant:   { id: 'diamant', emoji: '💎', nom: 'Diamant Pur', cout: 500, message: 'un diamant éclatant' },
  couronne:  { id: 'couronne', emoji: '👑', nom: 'Couronne Royale', cout: 1000, message: 'une couronne majestueuse' },
...
const maseratiOuvrirCoffretQuotidien = (userId) => {
  ...
  if (profil.derniereCoffretQuotidien) {
    const tempsEcoule = maintenant - profil.derniereCoffretQuotidien;
    if (tempsEcoule < CAIXAS_MASERATI.commune.delai) {
      const restant = CAIXAS_MASERATI.commune.delai - tempsEcoule;
  ...
  profil.derniereCoffretQuotidien = maintenant;
  const recompense = tirerRecompense(CAIXAS_MASERATI.commune);
...
const maseratiOuvrirCoffret = (userId, typeCoffret, orUtilisateur) => {
  const coffret = CAIXAS_MASERATI[typeCoffret];
  if (!coffret) {
...
const maseratiEnvoyerCadeau = (idEnvoyeur, idDestinataire, typeCadeau) => {
  ...
  const cadeau = CADEAUX_ENVOYABLES[typeCadeau.toLowerCase()];
  if (!cadeau) {
    const liste = Object.values(CADEAUX_ENVOYABLES)
...
const maseratiListerCadeaux = (prefixe = '/') => {
  ...
  Object.values(CADEAUX_ENVOYABLES).forEach(c => {
